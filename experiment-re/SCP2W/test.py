...
from models.CA_MMDG import DG_model
import shutil
...
def test(test_dataloader, model, threshold):
    ...
    with torch.no_grad():
        for iter, (input, target, videoID) in enumerate(test_dataloader):
            ...
            cls_out, _,_ = model(input, config.norm_flag)
...
def main():
    ...
    test_dataloader = DataLoader(FDDataset_wmca(mode='val',image_size=112), batch_size=2, shuffle=False)
    print('\n')
