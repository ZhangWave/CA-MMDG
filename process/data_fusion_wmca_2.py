...
class FDDataset_wmca_2(Dataset):
    def __init__(self, mode, modality='color', fold_index='<NIL>', image_size=128, augment = None, balance = True, crop=False, gray=True):
        super(FDDataset_wmca_2, self).__init__()
...
def run_check_train_data():
    dataset = FDDataset_wmca_2(mode = 'val')
    print(dataset)
    ...
    for m in range(num):
        ...
        print(image.shape)
        print(label)
        # 移除批量维度，将张量从 [1, 9, 112, 112] 变为 [9, 112, 112]
        image_np = image.squeeze(0).numpy()
        print("image_np.shape:", image_np.shape)  # 打印形状以确认变换是否正确

        # 将张量从 [9, 112, 112] 转换为 [112, 112, 9]
        image_np = np.transpose(image_np, (1, 2, 0))

        # 分离出3张3通道图像
        color_image = image_np[:, :, 0:3]
        depth_image = image_np[:, :, 3:6]
        ir_image = image_np[:, :, 6:9]
        ...
        axes[2].imshow(ir_image)
        axes[2].set_title('IR Image')
